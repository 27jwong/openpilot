...
class Streamer:
  ...
  def _get_ice_configuration(self) -> RTCConfiguration:
    try:
      # read the dongle id here, not at startup: athenad can start before registration finishes
      token = Api(self.params.get("DongleId")).get_token()
      ice_servers_data = api_get("v1/iceservers", timeout=5, access_token=token).content
      ice_servers = [
